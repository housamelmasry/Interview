...
• مثالي للبيانات ضخمة وغير منتظمة
  ...
  db.serialize(() => {
    ...
    db.run("DELETE FROM categories", () => {
      ...
      categories.forEach((cat) => {
        categoryStmt.run(cat.id, cat.label, cat.icon, cat.color);
        categoriesInserted++;

        cat.questions.forEach((q) => {
          questionStmt.run(cat.id, q.q, function (err) {
            if (err) {
      ...
      setTimeout(() => {
        ...
        console.log(`Total categories: ${categoriesInserted}`);
        console.log(`Total questions: ${questionsInserted}`);
        console.log(`Total answers: ${answersInserted}`);
        process.exit(0);
      }, 2000);
    });
  });
